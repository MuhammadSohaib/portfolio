...
st.set_page_config(page_title='MuhammadSohaib' ,layout="wide",page_icon='👧🏻')
...
def gradient2(color1, color2, color3, content1):
    st.markdown(f'<h3 style="text-align:left;background-image: linear-gradient(to right,{color1}, {color2});font-size:40px; padding-left: 20px; padding-right: 20px; line-height: 15px; border-radius:3%;">'
                f'<span style="color:white;font-size:17px;">{content1}</span></h3>', 
                unsafe_allow_html=True)

def styled_container(background_color="#ffffff", text_color="black"):
    """
    Create a container with custom styles.

    Args:
    background_color (str): The background color of the container.
    text_color (str): The color of the text inside the container.
    """
    style = f"""
    <style>
        .custom-container {{
...
with col2:
    st_lottie(lottie_gif, height=175, key="data")
...
# load the file
documents = SimpleDirectoryReader(input_files=["app/bio.txt"]).load_data()
...
def ask_bot(input_text):
    ...
    if not output.response or output.response.isspace() or "I don't know" in output.response or "I'm sorry" in output.response:
        message = f"""Sorry, I do not have enough information to answer this question. You can directly reach out to {name}'s' email: {info['Email']} Linkedin: {info['Linkedin']}"""
        return message
    else:
...
with st.container():
    ...
    with col7:
        st.markdown("##### Algorithm Expertise")
